...
面向 npm registry 的高并发 HTTP 路径转发代理。对每个请求**并发竞速 HEAD 探测**多个上游，再从**最高优先级的健康上游**下载，失败自动回退到下一个。每个上游可单独配置固定 IP 解析与 HTTP/SOCKS5(S) 代理（支持鉴权）。

## 特性

- 并发 HEAD 竞速，发现当前哪些上游可服务该资源
- 按优先级下载，失败自动回退到下一个健康上游
- 宽限窗口：首个 HEAD 成功后，给其余上游一小段额外时间再忽略它们
...
> `priority` 的语义是**数值越小越优先**。想让某个上游排在最前，就给它最小的数。
